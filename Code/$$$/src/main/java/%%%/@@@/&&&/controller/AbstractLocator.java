...
public abstract class AbstractLocator implements Locator {
    ...
    public final synchronized void locate(LocatableCommand locatableCommand, Locator.LocatorCallback callback){
        ...
        Log.d($$Service.TAG, this.getClass().getSimpleName() +":" + locatorId.intValue() + " locating " + locatableCommand);
        Locator.LocatorCallback finalCallback = callback;
        new Handler().post(() -> this.locate$$(locatableCommand, finalCallback, locatorId.intValue()));
    }
    ...
    private synchronized void  locate$$(LocatableCommand locatableCommand, Locator.LocatorCallback callback, int myLocatorId){
        if(locatableCommand == null || locatableCommand.getTargetWidgetInfo() == null)
        {
            Log.e($$Service.TAG, this.getClass().getSimpleName()+":" + myLocatorId + " locating null!");
            callback.onError("The locatableStep is null");
            return;
        }
        Log.d($$Service.TAG, this.getClass().getSimpleName() +":" + myLocatorId + " locating " + locatableCommand);
        if(myLocatorId != locatorId.intValue()){
            Log.d($$Service.TAG, this.getClass().getSimpleName() + " is interrupted!");
            callback.onError("The targetWidget is null");
            return;
        }
        if (locatableCommand.reachedMaxLocatingAttempts()){
            Log.d($$Service.TAG, "Reached max attempt for " + this.getClass().getSimpleName() + " locating " + locatableCommand);
        ...
        {
            Log.d($$Service.TAG, this.getClass().getSimpleName() +":" + myLocatorId + " is FAILED!");
        ...
        {
            Log.d($$Service.TAG, this.getClass().getSimpleName() +":" + myLocatorId+ " is COMPLETED!");
            ...
            Log.d($$Service.TAG, this.getClass().getSimpleName() +":" + myLocatorId + " is WAITING! Attempt: " + locatableCommand.getNumberOfLocatingAttempts());
            new Handler().postDelayed(() -> this.locate$$(locatableCommand, callback, myLocatorId), delay);
            return;
        }
    ...
    protected ActualWidgetInfo findActualWidget(ConceivedWidgetInfo targetWidget){
        ...
        if(similarNodes.size() != 1){
            if(similarNodes.size() == 0) {
                Log.e($$Service.TAG, "The target widget could not be found in current screen.");
                // TODO: For debugging
                Log.d($$Service.TAG, "The target XPATH: \n\t" + targetWidget.getXpath());
                ...
                for(AccessibilityNodeInfo nodeInfo : allNodes){
                    ...
                    if (actualWidgetInfo != null)
                        Log.d($$Service.TAG, "\t" + actualWidgetInfo.getXpath());
                }
            }
            else{
                Log.d($$Service.TAG, "There are more than one candidates for the target.");
                for(AccessibilityNodeInfo node : similarNodes){
                    Log.d($$Service.TAG, " Node: " + node);
                }
            }
            return null;
        }
