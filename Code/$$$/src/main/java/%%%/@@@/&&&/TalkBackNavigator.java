...
public class TalkBackNavigator {
    ...
    public boolean performNext(ActionUtils.ActionCallback doneCallback){
        Log.i($$Service.TAG, "performNext");
        if (!ActionUtils.swipeRight(doneCallback))
        {
            Log.i($$Service.TAG, "There is a problem with swiping right");
            return false;
        }
        return true;
    }
    ...
    public boolean performPrev(ActionUtils.ActionCallback doneCallback){
        Log.i($$Service.TAG, "performPrev");
        if (!ActionUtils.swipeLeft(doneCallback))
        {
            Log.i($$Service.TAG, "There is a problem with swiping left");
            return false;
        }
        return true;
    }
    ...
    public boolean logTalkBackTreeNodeList(ActionUtils.ActionCallback doneCallback){
        Log.i($$Service.TAG, "perform Up then Left");
        if (!ActionUtils.swipeUpThenLeft(doneCallback))
        {
            Log.i($$Service.TAG, "There is a problem with swiping Up then Left");
            return false;
        }
        return true;
    }
    ...
    public boolean performSelect(ActionUtils.ActionCallback doneCallback){
        Log.i($$Service.TAG, "performSelect");
        if(isPending()){
        ...
        AccessibilityService.GestureResultCallback callback = new AccessibilityService.GestureResultCallback() {
            ...
            public void onCancelled(GestureDescription gestureDescription) {
                Log.i($$Service.TAG, "Performing Select is cancelled!");
                pendingActions.remove(thisActionId);
                if(doneCallback != null)
    ...
    public boolean selectFocus(ActionUtils.ActionCallback doneCallback) {
        ...
        boolean result = performSelect(new ActionUtils.ActionCallback() {
            ...
            public void onCompleted(AccessibilityNodeInfo nodeInfo) {
                ...
                Log.i($$Service.TAG, "The focused node is tapped: " + focusedNode);
            ...
            public void onError(String message) {
                Log.i($$Service.TAG, "Cancel in double tapping!");
                if(doneCallback != null)
    ...
    public boolean changeFocus(ActionUtils.ActionCallback callback, boolean prev) {
        ...
        Log.i($$Service.TAG, String.format("Widget %s is visited XPATH: %s.", widgetInfo, widgetInfo != null ? widgetInfo.getAttr("xpath") : "NONE"));
        ActionUtils.ActionCallback afterChangeCallback = new ActionUtils.ActionCallback() {
            ...
            public void onCompleted(AccessibilityNodeInfo nodeInfo) {
                ...
                if (newWidgetNodeInfo == null) {
                    Log.i($$Service.TAG, "The next focused node is null");
                    jsonCommandStr = "Error";
                }
                else {
                    Log.i($$Service.TAG, "The next focused node is: " + newWidgetNodeInfo + " Xpath: " + newWidgetNodeInfo.getXpath());
            ...
            @Override
            public void onError(String message) {
                Log.i($$Service.TAG, "Error in next: " + message);
                if(callback != null)
    ...
    public void currentFocus() {
        ...
        WidgetInfo widgetInfo = ActualWidgetInfo.createFromA11yNode($$Service.getInstance().getAccessibilityFocusedNode());
        if (widgetInfo != null) {
            Log.i($$Service.TAG, "The focused node is: " + widgetInfo + (widgetInfo != null ? " Xpath: " + widgetInfo.getXpath() : ""));
