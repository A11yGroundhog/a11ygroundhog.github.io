...
public class RegularStepExecutor implements StepExecutor {
    ...
    public boolean executeStep(Command step) {
        Log.i($$Service.TAG, "Reg Executing Step " + step + " Physical Touch: " + is_physical);
    ...
    private boolean executeClick(ClickCommand clickStep, AccessibilityNodeInfo node){
        if(is_physical){
            ...
            if(!clickResult){
                Log.e($$Service.TAG, "The location could not be clicked.");
            ...
            Log.i($$Service.TAG, "Clicking on widget: " + clickableWidget.completeToString(true));
