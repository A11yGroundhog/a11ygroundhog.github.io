...
public class ActionUtils {
    ...
    static class DefaultGestureCallback extends AccessibilityService.GestureResultCallback {
        ...
        public void onCompleted(GestureDescription gestureDescription) {
            Log.i($$Service.TAG, "Complete Gesture " + gestureDescription.toString());
        ...
        public void onCancelled(GestureDescription gestureDescription) {
            Log.i($$Service.TAG, "Cancel Gesture " + gestureDescription.toString());
            if (actionCallback != null)
    ...
    public static boolean isFocusedNodeTarget(List<AccessibilityNodeInfo> similarNodes) {
        ...
        boolean isSimilar = firstReachableNode != null && firstReachableNode.equals($$Service.getInstance().getAccessibilityFocusedNode());
        if(!isSimilar) {
            ...
            Log.i($$Service.TAG, "-- FIRST REACHABLE NODE IS " + firstReachableNode);
            isSimilar = firstReachableNode != null && firstReachableNode.equals($$Service.getInstance().getAccessibilityFocusedNode());
        }
        return isSimilar;
    }
    ...
    public static Pair<Integer, Integer> getClickableCoordinate(AccessibilityNodeInfo node, boolean excludeDescendantsBounds){
        ...
        if(excludeDescendantsBounds)
        {
            ...
            Log.i($$Service.TAG, " -------> " + nodeBox + " " + firstDescendantLeft + " " + lastDescendantRight + " " + firstDescendantTop + " " + lastDescendantBottom);
    ...
    public static boolean performTap(int x, int y, int startTime, int duration, AccessibilityService.GestureResultCallback callback){
        ...
        Log.i($$Service.TAG, "Execute Gesture " + gestureDescription.toString());
        return $$Service.getInstance().dispatchGesture(gestureDescription, callback, null);
    }

    public static boolean performType(AccessibilityNodeInfo node, String message){
        Log.i($$Service.TAG, "performType");
    ...
    public static boolean performDoubleTap(final AccessibilityService.GestureResultCallback callback){
        Log.i($$Service.TAG, "performDoubleTap");
        try {
    ...
    public static boolean performDoubleTap(final int x, final int y, final int startTime, final int duration, final AccessibilityService.GestureResultCallback callback){
        AccessibilityService.GestureResultCallback newClickCallBack = new AccessibilityService.GestureResultCallback() {
            @Override
            public void onCompleted(GestureDescription gestureDescription) {
                Log.i($$Service.TAG, "Complete Gesture " + gestureDescription.getStrokeCount());
                super.onCompleted(gestureDescription);
                try {
            ...
            @Override
            public void onCancelled(GestureDescription gestureDescription) {
                Log.i($$Service.TAG, "Cancel Gesture");
                super.onCancelled(gestureDescription);
                if(callback != null)
    ...
    private static void executeSwipeGesture(String direction, String secondDirection, AccessibilityService.GestureResultCallback callback){
        ...
        int x1, x2, y1, y2;
        WindowManager wm = (WindowManager) $$Service.getInstance().getApplicationContext().getSystemService(Context.WINDOW_SERVICE);
        ...
        switch (direction) {
            ...
            case "down":
                ...
                break;
            default:
                Log.e($$Service.TAG, "Incorrect direction " + direction);
                return;
        }
        ...
        if (!secondDirection.isEmpty())
        {
            ...
            {
                Log.i($$Service.TAG, "Add left direction to " + Integer.toString(100+dx1) + " " +Integer.toString(y2-dy1));
        ...
        GestureDescription gestureDescription = gestureBuilder.build();
        Log.i($$Service.TAG, "Execute Gesture " + gestureDescription.toString());
        $$Service.getInstance().dispatchGesture(gestureDescription, callback, null);
    }
    ...
    public static boolean swipeToDirection(String direction, String secondDirection, ActionCallback doneCallback){
        if(isActionPending()){
            Log.i($$Service.TAG, String.format("Do nothing since another action is pending! (Size:%d)", pendingActions.size()));
            return false;
        }
        ...
        AccessibilityService.GestureResultCallback callback = new AccessibilityService.GestureResultCallback() {
            ...
            public void onCompleted(GestureDescription gestureDescription) {
                new Handler().postDelayed(() -> {
                    ...
                    if(doneCallback != null)
                        doneCallback.onCompleted($$Service.getInstance().getAccessibilityFocusedNode());
            ...
            @Override
            public void onCancelled(GestureDescription gestureDescription) {
                Log.i($$Service.TAG, "Gesture is cancelled!");
                pendingActions.remove(thisActionId);
                if(doneCallback != null)
    ...
    public static boolean a11yFocusOnNode(AccessibilityNodeInfo node){
        AccessibilityNodeInfo currentFocusedNode = $$Service.getInstance().getAccessibilityFocusedNode();
        ...
        if (focusableWidget == null) {
            Log.e($$Service.TAG, "The requested focusing  widget is null!");
            return false;
        }
        Log.i($$Service.TAG, "Focusing on widget: " + focusableWidget.completeToString(true));
    ...
    public static boolean focusOnNode(AccessibilityNodeInfo node){
        AccessibilityNodeInfo currentFocusedNode = $$Service.getInstance().getFocusedNode();
        if (currentFocusedNode != null)
