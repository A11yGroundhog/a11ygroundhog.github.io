...
import asyncio
import aiofiles
from consts import ---_%%%_TAG, CAPTURE_SCREENSHOT_DELAY


async def save_screenshot(&&&ice, file_name) -> None:
    result = await &&&ice.screencap()
...
class ParallelADBLogger:
    ...
    async def _logcat(self) -> asyncio.coroutine:
        ...
        conn = await self.&&&ice.create_connection(timeout=None)
        cmd = "shell:{}".format("logcat -c; logcat")
        await conn.send(cmd)
        logcat_handler_$$ = asyncio.create_$$(logcat_handler(conn))
        return logcat_handler_$$
