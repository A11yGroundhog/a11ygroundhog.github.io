...
from $$_utils import send_commands_sequence_to_$$, send_command_to_$$
...
class Controller(ABC):
    ...
    async def execute(self, command: Command, first_setup: bool = False) -> CommandResponse:
        ...
        for i in range(ACTION_EXECUTION_RETRY_COUNT):
            ...
            await send_command_to_$$(command="controller_execute", extra=command.toJSONStr())
            ...
            if result is None:
                ...
                await send_command_to_$$("controller_interrupt")
            else:
...
class TouchController(Controller):
    async def setup(self):
        ...
        await send_commands_sequence_to_$$([("controller_set", "touch")])


class A11yAPIController(Controller):
    async def setup(self):
        await A11yServiceManager.setup_$$_a11y_services(tb=False)
        await send_commands_sequence_to_$$([("controller_set", "a11y_api")])


class TalkBackAPIController(Controller):
    async def setup(self):
        await A11yServiceManager.setup_$$_a11y_services(tb=True)
        await send_commands_sequence_to_$$([("controller_set", "tb_api")])


class TalkBackTouchController(Controller):
    async def setup(self):
        await A11yServiceManager.setup_$$_a11y_services(tb=True)
        await send_commands_sequence_to_$$([("controller_set", "tb_touch")])
