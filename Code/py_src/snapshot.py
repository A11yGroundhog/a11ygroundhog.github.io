...
from ppadb.&&&ice_async import &&&iceAsync
...
class &&&iceSnapshot(Snapshot):
    def __init__(self, address_book: AddressBook, &&&ice: &&&iceAsync = None):
        super().__init__(address_book=address_book)
        if &&&ice is None:
            client = AdbClient(host=ADB_HOST, port=ADB_PORT)
            &&&ice = asyncio.run(client.&&&ice(&&&ICE_NAME))
        self.&&&ice = &&&ice
    ...
    async def setup(self, first_setup: bool = True, dumpsys: bool = True, use_service: bool = True, **kwargs):
        ...
        if first_setup:
            if use_service:
                await A11yServiceManager.setup_$$_a11y_services(tb=False)
...
class EmulatorSnapshot(&&&iceSnapshot):
    def __init__(self, address_book: AddressBook, &&&ice: &&&iceAsync = None, no_save_snapshot: bool = False):
        super().__init__(address_book=address_book, &&&ice=&&&ice)
    ...
    async def setup(self, first_setup: bool = True, initial_emulator_load: bool = False, **kwargs):
        if initial_emulator_load:
            if not await load_snapshot(self.address_book.snapshot_name(), &&&ice_name=self.&&&ice.serial):
    ...
    async def reload(self, hard: bool = False) -> bool:
        ...
        if self.no_save_snapshot:
            ...
            return False
        result = await load_snapshot(self.tmp_snapshot, &&&ice_name=self.&&&ice.serial)
        await asyncio.sleep(3)
        return result
