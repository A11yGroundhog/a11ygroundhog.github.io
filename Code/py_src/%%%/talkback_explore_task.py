...
from consts import ---_%%%_TAG, ---_%%%_EVENTS_TAG, EXPLORE_VISIT_LIMIT, MAX_DIRECTIONAL_NAVIGATION
...
from snapshot import &&&iceSnapshot, EmulatorSnapshot
from $$.snapshot_$$ import Snapshot$$
...
def is_window_changed(log_message_map):
    try:
        ...
        for line in log_message_map[---_%%%_EVENTS_TAG].split("\n"):
...
class TalkBackExplore$$(Snapshot$$):
    def __init__(self, snapshot: &&&iceSnapshot, check_both_directions: bool = False):
        if not isinstance(snapshot, &&&iceSnapshot):
            raise Exception("TalkBack exploration requires a &&&iceSnapshot!")
        super().__init__(snapshot)
        self.check_both_directions = check_both_directions

    async def execute(self):
        snapshot: &&&iceSnapshot = self.snapshot
        controller = TalkBackAPIController()
        await controller.setup()
        padb_logger = ParallelADBLogger(snapshot.&&&ice)
        is_next = True
        all_nodes = {node.xpath: node for node in self.snapshot.get_nodes()}
        visited_node_xpaths_counter = defaultdict(int)
        self.snapshot.address_book.initiate_talkback_explore_$$()
        ...
        android_logs += f"--------- First Focused Node -----------\n" \
                        f"{log_message_map[---_%%%_TAG]}\n" \
                        f"----------------------------------------\n"
        android_event_logs += f"{log_message_map[---_%%%_EVENTS_TAG]}\n"
