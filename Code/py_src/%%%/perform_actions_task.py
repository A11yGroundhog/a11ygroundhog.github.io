...
from $$_executor_utils import report_atf_issues
...
class PerformActions$$(Snapshot$$):
    def __init__(self, snapshot: EmulatorSnapshot):
        if not isinstance(snapshot, EmulatorSnapshot):
            raise Exception("Perform Actions $$ requires a EmulatorSnapshot!")
    ...
    async def execute(self):
        ...
        if not snapshot.address_book.audit_path_map[AddressBook.EXTRACT_ACTIONS].exists():
            ...
            return
        snapshot.address_book.initiate_perform_actions_$$()
        controllers = {
